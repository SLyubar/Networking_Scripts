...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    timeLeft = timeout
    # global packet_min, packet_max, packet_sum, packet_cnt
...
def ping(host, timeout=1):
    ...
    # packet_cnt= 0
    # cnt = 0
    # packet_avg = packet_sum / packet_cnt
    # rtt_min = float('+inf')
    # rtt_max = float('-inf')
    # rtt_sum = 0
    # rtt_cnt = 0
    ...
    for i in range(0,4):
        ...
        packet.append(i)
    packet_min = min(packet) * 1000
    packet_max = max(packet) * 1000
    packet_avg = (sum(packet)/len(packet)) * 1000
    stdev_var = packet * 1000
    vars = [str(round(packet_min, 2)), str(round(packet_avg, 2)), str(round(packet_max, 2)),str(round(stdev(stdev_var), 2))]
    return vars
